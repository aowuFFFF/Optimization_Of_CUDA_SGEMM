...
__global__ void mysgemm_v9(int M, int N, int K, float alpha, float *A, float *B, float beta, float *C) {
    ...
    // accum[TM][TN]用于存放每个线程计算的部分结果。
    float accum[TM][TN] = {0.}; // 每个线程负责TM*TN个元素，则需要申请TM*TN个寄存器保存累加值，额外的一个寄存器用于缓存；

    // ldg_a_reg和ldg_b_reg分别是用于暂存从全局内存加载的A和B矩阵的数据。
    ...
    // a_frag和b_frag则是用于存储当前线程块的A和B矩阵片段。
...
#pragma unroll
    ...
    int write_index = 1; // write_index用于切换到另一个缓存槽。
    int load_index;      // load_index用于确定当前正在使用的缓存槽
    int k = 0;
    do {
        k += BK;
        // load global to reg
...
#pragma unroll
        ...
        load_index = write_index ^ 1; //获取另一个缓存槽的索引
#pragma unroll
...
#pragma unroll
            ...
            __syncthreads(); // 这些同步点保证了在开始下一轮加载和计算之前，所有线程都完成了他们的任务，避免了数据竞争和不一致的问题。
#pragma unroll
            for (int m = 0; m < TM; m += 4) {

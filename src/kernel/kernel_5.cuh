...
__global__ void mysgemm_v5(int M, int N, int K, float alpha, float *A, float *B, float beta, float *C) {
    ...
    */
    // 用于确定线程在A矩阵的tile中的行和列位置。
    int a_tile_row = threadIdx.x / BK;  
    int a_tile_col = threadIdx.x % BK;
    int a_tile_stride = thread_num / BK; // BK为列，计算出每个线程需要在全局内存中搬运多少行数据。

    // 用于确定线程在B矩阵的tile中的行和列位置。
    int b_tile_row = threadIdx.x / BN;
    int b_tile_col = threadIdx.x % BN;
    int b_tile_stride = thread_num / BN; // BN为列

    float tmp[TM][TN] = {0.}; // 每个线程负责TM*TN个元素，则需要申请TM*TN个寄存器保存累加值，额外的一个寄存器用于缓存；
    #pragma unroll
    for (int k = 0; k < K; k += BK) {
        #pragma unroll
        // BM 是矩阵 A分块 的块高度，i这里表示加载的行数。
        // 步长 a_tile_stride: 步长决定了每次循环时跳过多少行。通过这种方式，可以有效地将矩阵的多行数据并行地加载到共享内存中。

...
# --------------------------------------------------------------------
# 概述：
# 1. 此脚本用于生成 BM、BN、BK、TM 和 TN 的不同组合，并在多种矩阵大小下评估其性能表现。
# 2. 运行时间将作为结果的一部分附加在输出文件的末尾。
#
# 运行方式：
#   ./kernel_8_autotuner.sh
#
# 输出文件：
#   结果将保存在 ./kernel_8_autotune_results.txt 中。
#
# --------------------------------------------------------------------
...
OUTPUT="../scripts/kernel_8_autotune_results.txt"
      ...
      for bm in ${BM_VALUES[@]}; do
        for bn in ${BN_VALUES[@]}; do
          for nt in ${NUM_THREADS_VALUES[@]}; do
            ...
            # # echo "8" |& tee -a $OUTPUT
            # /home/ubuntu/yujie/NVIDIA_SGEMM_PRACTICE-master/sgemm 8 | tee -a $OUTPUT
